...
class E:
    ...
    def result(self, MainWindow,sid):

        def rej(self) :
             MainWindow.close();              
        def acc(self) :
             contact=self.lineEdit_10.text();
             ...
             insert.big(Sid,name,address,gender,dob,age,email,contact,course,course_id,year,ssc,cet,hsc,jee,diploma,sem_1,sem_2,sem_3,sem_4,sem_5,sem_6,sem_7,sem_8);
             msg=QMessageBox()
        ...
        self.buttonBox.accepted.connect(lambda :acc(self));
        self.buttonBox.accepted.connect(lambda :rej(self));
